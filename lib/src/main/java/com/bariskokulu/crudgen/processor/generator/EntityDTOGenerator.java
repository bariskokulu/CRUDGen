...
public class EntityDTOGenerator {
	...
	private static void generateOne(EntityElement element, DTOElement dtoElement, ProcessingEnvironment processingEnv) {
		...
		for (DTOFieldElement f : dtoElement.getFields()) {
			clazz.addField(FieldSpec.builder(ClassName.get(f.getType()), f.getName(), Modifier.PRIVATE, Modifier.FINAL).build());
		}
		MethodSpec.Builder ctor = MethodSpec.constructorBuilder().addModifiers(Modifier.PUBLIC);
		for (DTOFieldElement f : dtoElement.getFields()) {
			ctor.addParameter(ParameterSpec.builder(ClassName.get(f.getType()), f.getName()).build());
