...
public class UseCaseControllerGenerator {
	...
	public static void generate(UseCaseServiceElement element, ProcessingEnvironment processingEnv) {
		...
		if (element.isLogging()) {
			clazz.addField(FieldSpec.builder(TypeNames.LOGGER, "logger", Modifier.PRIVATE, Modifier.STATIC, Modifier.FINAL)
					.initializer("$T.getLogger($T.class)", TypeNames.LOGGER_FACTORY, element.getControllerTypeName()).build());
		}
		...
		MethodSpec.Builder constructorBuilder = MethodSpec.constructorBuilder().addModifiers(Modifier.PUBLIC)
				.addParameter(element.getTypeName(), "service")
		...
		for (EndpointElement endpoint : element.getEndpoints()) {
			MethodSpec.Builder method = MethodSpec.methodBuilder(endpoint.getName())
					...
					.addAnnotation(AnnotationSpec.builder(ClassName.bestGuess(
							"org.springframework.web.bind.annotation." + endpoint.getHttpMethod().text() + "Mapping"))
			...
			for (ParameterElement param : endpoint.getParams()) {
				...
				for (javax.lang.model.element.AnnotationMirror mirror : param.getElement().getAnnotationMirrors()) {
					if (!Util.isSpringWebBindingAnnotation(mirror)) {
						paramBuilder.addAnnotation(AnnotationSpec.get(mirror));
					}
				}
				String paramLocation = determineParameterType(param, endpoint.getPath());
				String bindingName = Util.springBindingName(param.getElement(), param.getName());
				if ("PATH".equals(paramLocation)) {
					paramBuilder.addAnnotation(Util.pathVariable(bindingName));
				} else if ("BODY".equals(paramLocation)) {
					paramBuilder.addAnnotation(AnnotationSpec.builder(TypeNames.REQUEST_BODY).build());
				} else {
					paramBuilder.addAnnotation(Util.requestParam(bindingName,
							isParameterRequired(param, endpoint.getPath(), paramLocation)));
				}
				if (element.isOpenApi()) {
					if ("BODY".equals(paramLocation)) {
						paramBuilder.addAnnotation(OpenApiUtil.buildRequestBodyAnnotation(param.getName()));
					} else {
						boolean isRequired = isParameterRequired(param, endpoint.getPath(), paramLocation);
						paramBuilder.addAnnotation(OpenApiUtil.buildParameterAnnotation(bindingName,
								"Parameter: " + bindingName, paramLocation, isRequired));
					}
				}
			...
			method.addCode(checkThenAddLog(element, endpoint.getName() + "() called with params: "
					+ (paramString.isEmpty() ? "no parameters" : paramString),
					endpoint.getParams().stream().map(ParameterElement::getName).toArray(String[]::new)));
			String useCaseKey = element.getTypeName().canonicalName() + "#" + endpoint.getName();
			method.addCode(checkThenAddSecurity(element, useCaseKey,
					endpoint.getParams().stream().map(ParameterElement::getName).collect(Collectors.joining(", "))));
			if (endpoint.getReturnType() != null) {
				...
				method.addStatement("return $T.noContent().build()", TypeNames.RESPONSE_ENTITY);
				method.returns(ParameterizedTypeName.get(TypeNames.RESPONSE_ENTITY, ClassName.get(Void.class)));
			}
	...
	public static String checkThenAddSecurity(UseCaseServiceElement element, String useCaseKey, String params) {
		return element.isSecureEndpoints()
				? "securityService.checkUseAccess(\"" + useCaseKey + "\"" + (params.length() > 0 ? ", " : "") + params + ");\n"
				: "";
	}
	...
	private static String determineParameterType(ParameterElement param, String endpointPath) {
		Element e = param.getElement();
		if (hasAnyQualifiedAnnotation(e, SPRING_PATH_VARIABLE)) {
			return "PATH";
		}
		if (Util.pathVariableNamesFromPath(endpointPath).contains(param.getName())) {
			return "PATH";
		}
	...
	private static boolean isParameterRequired(ParameterElement param, String endpointPath, String paramLocation) {
		Element e = param.getElement();
		if ("PATH".equals(paramLocation) || hasAnyQualifiedAnnotation(e, SPRING_REQUEST_BODY)) {
			return true;
		}
