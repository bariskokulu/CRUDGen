...
public class UseCaseControllerGenerator {
		...
		for (EndpointElement endpoint : element.getEndpoints()) {
			...
			for (ParameterElement param : endpoint.getParams()) {
				...
				if ("PATH".equals(paramLocation)) {
					...
					paramBuilder.addAnnotation(Util.requestParam(bindingName, isParameterRequired(param, endpoint.getPath(), paramLocation)));
			...
			method.addCode(checkThenAddSecurity(element, useCaseKey, endpoint.getParams().stream().map(ParameterElement::getName).collect(Collectors.joining(", "))));
	...
	public static String checkThenAddSecurity(UseCaseServiceElement element, String useCaseKey, String params) {
		...
		return element.isSecureEndpoints() ? "securityService.checkUseAccess(\"" + useCaseKey + "\"" + (params.length() > 0 ? ", " : "") + params + ");\n" : "";
